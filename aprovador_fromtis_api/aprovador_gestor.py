...
def retrieve_list_operacoes_aprovar(dict_credencial):
    print("=" * 80)
    print("RETRIEVING OPERATIONS TO APPROVE")
    print("=" * 80)
...
def consulta_operacao():
    ...
    shuffle(list_operacoes_aprovar)
    ...
    for dict_operacao in reversed(list_operacoes_aprovar):
        ...
        if DICT_CODIGO_SITUCAO[dict_operacao['codigoSituacaoOperacao']].upper() == 'AGUARDANDO APROVAÇÃO DO GESTOR' and \
            ...
            print("\n" + "=" * 80)
            print("🎯 OPERATION MATCHES APPROVAL CRITERIA!")
            print("=" * 80)
            ...
            response = None
            try:
                print("Calling SOAP service aprovarGestor...")
                response = client.service.aprovarGestor(request_data)
...
def main():
    ...
    # Otherwise, use the provided argument or look in the same directory as the script
    if len(sys.argv) > 1:
        ...
        config_path = application_path / 'mysql.json'

    print(f"📁 Loading configuration from: {config_path}")
    dict_config_mysql = json.loads(config_path.read_text())
    ...
    while True:
        ...
        print("\n\n")
        print("🔄" * 40)
        print(f"🔄 ITERATION #{iteration} - Checking operations to approve")
